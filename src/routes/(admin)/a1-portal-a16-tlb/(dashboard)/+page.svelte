<script lang="ts">
	...
	const stats = $derived(dashboardData ? [
		{
			title: 'Total Revenue',
		...
		},
		{
			...
			value: dashboardData.general.totalReservation.toString(),
			icon: CalendarCheck,
			color: 'text-blue-400',
			bg: 'bg-blue-400/10'
		},
		{
			...
			value: dashboardData.general.activeBarbers.toString(),
			icon: Scissors,
...
<div class="min-h-screen w-full bg-slate-950 text-secondary selection:bg-senary/30">
	<AdminHeader
		pageName="Dashboard"
		description="Overview of your barbershop performance"
	/>
		...
		<div class="mx-auto max-w-7xl space-y-8">
			...
			{:else if dashboardData}
				...
				<div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
					{#each stats as stat}
						<div class="group relative overflow-hidden rounded-3xl border border-white/5 bg-white/5 p-6 backdrop-blur-md transition-all duration-300 hover:border-senary/30 hover:bg-white/10 hover:shadow-[0_0_30px_-10px_rgba(212,175,55,0.1)]">
							<div class="flex items-center justify-between">
								<div>
									<p class="text-sm font-medium text-secondary/60 uppercase tracking-wider">{stat.title}</p>
									<h3 class="mt-2 text-3xl font-light text-secondary group-hover:text-senary transition-colors">{stat.value}</h3>
								</div>
				...
				<div class="grid gap-6 lg:grid-cols-3">
					<!-- Revenue Chart -->
					<div class="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-md lg:col-span-2">
						...
						<div class="h-[350px] w-full">
							...
							>
								<Svg>
									<Axis placement="left" grid rule class="stroke-white/30 fill-[#F5F5DC] text-[10px]" format={(v) => formatCurrency(v).replace('Rp', '')} />
									<Axis placement="bottom" rule class="stroke-white/30 fill-[#F5F5DC] text-xs" />
									<Bars radius={6} strokeWidth={0} class="fill-senary transition-all hover:fill-senary/80" />
								</Svg>
								<Tooltip.Root let:data>
									<Tooltip.Header class="text-secondary font-bold">{data.x}</Tooltip.Header>
									<Tooltip.List>
										<Tooltip.Item label="Revenue" value={data.y} format="currency" class="text-senary" />
									</Tooltip.List>
								</Tooltip.Root>
							</Chart>
						</div>
					...
					<div class="space-y-6">
						...
						<div class="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-md h-fit">
							<div class="mb-4 flex items-center justify-between">
								...
								<Badge variant="outline" class="border-yellow-500/20 text-yellow-500 bg-yellow-500/10">
							...
							<ScrollArea class="h-[200px] pr-4">
								<div class="space-y-3">
									{#each dashboardData.general.pendingCorrections as item}
										<div class="flex items-center justify-between rounded-xl border border-white/5 bg-black/20 p-3">
											<div>
							...
							<ScrollArea class="h-[200px] pr-4">
								<div class="space-y-3">
									...
									{#if dashboardData.general.upcomingReservations.length === 0}
										<p class="text-center text-xs text-secondary/40 py-4">No upcoming reservations</p>
									{/if}
								</div>
							</ScrollArea>
						</div>
				...
				<div class="rounded-3xl border border-white/10 bg-white/5 p-8 backdrop-blur-md">
					<div class="mb-6 flex items-center justify-between">
						...
						<Button variant="outline" href="/a1-portal-a16-tlb/Reservation" class="border-senary/50 text-senary hover:bg-senary hover:text-primary">
							View All
						</Button>
					</div>
									...
									<tr class="hover:bg-white/5 transition-colors">
										<td class="px-6 py-4">
											<div class="flex items-center gap-3">
												<div class="flex h-8 w-8 items-center justify-center rounded-full bg-senary/10 text-xs font-bold text-senary">
										...
										<td class="px-6 py-4">
											<div class="flex flex-col">
												<span class="text-secondary">{format(new Date(reservation.date), 'dd MMM yyyy')}</span>
												<span class="text-xs text-secondary/50">{reservation.time}</span>
											</div>
										</td>
										<td class="px-6 py-4">
											<Badge variant="outline" class={
												reservation.status === 'completed' ? 'border-green-500/20 text-green-500 bg-green-500/10' :
												reservation.status === 'pending' ? 'border-yellow-500/20 text-yellow-500 bg-yellow-500/10' :
												reservation.status === 'confirmed' ? 'border-blue-500/20 text-blue-500 bg-blue-500/10' :
