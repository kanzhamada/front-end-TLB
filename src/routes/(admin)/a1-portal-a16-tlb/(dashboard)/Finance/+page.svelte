...
<div class="h-[calc(100vh-6rem)] flex flex-col gap-4 p-6 max-w-[1920px] mx-auto overflow-hidden">
		...
		<div class="flex items-center gap-3">
			<div class="relative group">
				<div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
					<Search class="h-3.5 w-3.5 text-secondary/40 group-focus-within:text-senary transition-colors" />
				</div>
				<Input 
					type="text" 
			...
			<Button variant="outline" size="sm" class="border-white/10 hover:bg-white/5 hover:text-white h-9 rounded-lg">
		...
		<div class="bg-white/5 rounded-xl p-3 border border-white/10 backdrop-blur-sm">
			...
			<div class="grid grid-cols-3 gap-3">
				<div class="bg-black/20 rounded-lg p-3 border border-white/5 hover:border-white/10 transition-colors">
					<p class="text-[10px] font-medium text-secondary/60 uppercase tracking-wider mb-1">Income</p>
				...
				<div class="bg-black/20 rounded-lg p-3 border border-white/5 hover:border-white/10 transition-colors">
					<p class="text-[10px] font-medium text-secondary/60 uppercase tracking-wider mb-1">Expenses</p>
					<div class="text-lg font-bold text-white tracking-tight">
				...
				<div class="bg-gradient-to-br from-senary/10 to-black/40 rounded-lg p-3 border border-senary/20 hover:border-senary/30 transition-colors">
	...
	<div class="grid grid-cols-1 2xl:grid-cols-2 gap-4 flex-1 min-h-0">
		<!-- Income Column -->
		<div class="flex flex-col bg-white/5 rounded-xl border border-white/10 backdrop-blur-sm overflow-hidden">
			...
			<div class="flex-1 overflow-hidden relative">
				<div class="absolute inset-0 overflow-y-auto no-scrollbar">
					<Table.Root>
						<Table.Header class="bg-black/20 sticky top-0 z-10 backdrop-blur-md border-b border-white/5">
							<Table.Row class="border-white/5 hover:bg-transparent">
								<Table.Head class="text-senary font-bold uppercase tracking-wider text-[10px] py-3 pl-4 h-auto">Date</Table.Head>
								<Table.Head class="text-senary font-bold uppercase tracking-wider text-[10px] py-3 h-auto">Service</Table.Head>
								<Table.Head class="text-senary font-bold uppercase tracking-wider text-[10px] py-3 h-auto">Type</Table.Head>
								<Table.Head class="text-senary font-bold uppercase tracking-wider text-[10px] py-3 text-right h-auto">Amount</Table.Head>
						...
						<Table.Body>
							{#each filteredIncome as item}
								...
								>
									...
									<Table.Cell class="text-secondary/80 text-xs py-2.5">{item.service.name}</Table.Cell>
									...
									<Table.Cell class="py-2.5">
										<Button variant="ghost" size="icon" class="h-6 w-6 text-secondary/50 hover:text-white hover:bg-white/10 rounded-full">
				...
				<div class="absolute inset-0 overflow-y-auto no-scrollbar">
					<Table.Root>
						<Table.Header class="bg-black/20 sticky top-0 z-10 backdrop-blur-md border-b border-white/5">
							<Table.Row class="border-white/5 hover:bg-transparent">
								<Table.Head class="text-senary font-bold uppercase tracking-wider text-[10px] py-3 pl-4 h-auto">Date</Table.Head>
								<Table.Head class="text-senary font-bold uppercase tracking-wider text-[10px] py-3 h-auto">Description</Table.Head>
								<Table.Head class="text-senary font-bold uppercase tracking-wider text-[10px] py-3 text-right h-auto">Amount</Table.Head>
								<Table.Head class="w-[40px] py-3 h-auto"></Table.Head>
							</Table.Row>
						</Table.Header>
						<Table.Body>
							{#each filteredExpenses as item}
								...
								>
									...
									<Table.Cell class="text-secondary/80 text-xs py-2.5">{item.description}</Table.Cell>
