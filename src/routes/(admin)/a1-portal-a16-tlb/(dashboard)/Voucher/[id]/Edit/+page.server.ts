...
import { createVoucher, editVoucher } from '$lib/api/admin/voucher.js';
...
export const actions: Actions = {
	default: async ({ request, locals, fetch }) => {
		...
		try {
			...
			if (data.id) {
				const updated = await editVoucher(fetch, data.id, { ...data, id: data.id }, token);
				if (!updated.success) throw new Error(updated.message || 'Failed to update');
			} else {
				if (!data.id) data.id = userId();
				const created = await createVoucher(fetch, { ...data, id: data.id }, token);
