		...
		<div class="max-h-[75vh] overflow-y-auto px-8 py-8 no-scrollbar">
				...
				<div class="flex flex-col items-center space-y-6">
					...
					<div class="text-center">
						...
						{#if (service.attainableCoin ?? 0) > 0}
							<div class="mt-3 inline-flex items-center rounded-full border border-yellow-500/20 bg-yellow-500/10 px-3 py-1">
								<Coins class="mr-2 h-3 w-3 text-yellow-500" />
								<span class="text-xs font-medium text-yellow-500">+{service.attainableCoin} Coins</span>
							</div>
						{/if}
					</div>
					...
					<div class="w-full space-y-4 rounded-2xl border border-white/5 bg-white/5 p-6">
						<div>
							...
							<p class="text-secondary leading-relaxed">{service.description || 'No description provided.'}</p>
						</div>
					</div>
			...
			{:else}
				...
				<form onsubmit={handleSubmit} class="space-y-6">
					<div class="space-y-2">
						<Label for="name" class="text-xs font-bold tracking-widest text-secondary/70 uppercase">Service Name</Label>
						<Input 
							id="name" 
							bind:value={name} 
					...
					<div class="grid gap-4 sm:grid-cols-2">
						<div class="space-y-2">
							<Label for="price" class="text-xs font-bold tracking-widest text-secondary/70 uppercase">Price (IDR)</Label>
							<Input 
								id="price" 
								type="number"
								bind:value={price} 
						...
						<div class="space-y-2">
							<Label for="coins" class="text-xs font-bold tracking-widest text-secondary/70 uppercase">Attainable Coins</Label>
							<Input 
								id="coins" 
								type="number"
