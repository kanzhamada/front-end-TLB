...
export function createStatusCellSnippet(
	statusMap: Record<string, { text: string; class: string }>
) {
	return createRawSnippet<[string]>((getStatus) => {
		...
		return {
			render: () =>
				`<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${config.class}">${config.text}</span>`
		};
	});
}
